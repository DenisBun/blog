...
- [x] Use English as the default locale, with Russian (`/ru/`) and Polish (`/pl/`) localized routes.
...
- [x] Keep one matching English/Russian/Polish example-post set and remove the other demo posts.
...
- [x] Configure `theme.config.ts`: site identity, author/publisher metadata, English/Russian/Polish i18n, blog behavior, disabled promotions, static rendering, and project-specific LLM text.
- [x] Add complete, key-compatible `en.json`, `ru.json`, and `pl.json` translation files; remove German, French, and Spanish.
- [x] Reduce the article collection to English/Russian/Polish and remove the events collection and event-specific sitemap/runtime logic.
...
- [x] Review Open Graph/X, canonical, hreflang, structured data, RSS, sitemap, robots, web manifest, and `llms.txt` output for all languages.
...
- [x] Run the production build and postbuild scripts; verify generated English/Russian/Polish routes and technical outputs.
...
- [x] Re-audit generated multilingual SEO output and add a build-time regression check for metadata, social cards, structured data, sitemap, RSS, and crawl directives.

## Follow-up enhancements

- [x] Add Polish as a complete third locale and replace the language toggle with an accessible dropdown.
